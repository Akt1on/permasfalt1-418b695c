...
function ServicePage() {
  ...
  const { data: service = staticService, isLoading } = useQuery({ queryKey: ["service", slug], queryFn: () => fetchService(slug), initialData: staticService, staleTime: Infinity, initialDataUpdatedAt: 0, initialDataUpdatedAt: 0 });
  const { data: pricing = [] } = useQuery({
    queryKey: ["pricing", service?.id], queryFn: () => fetchPricing(service!.id), enabled: !!service?.id,
  });
  const { data: services = STATIC_SERVICES } = useQuery({ queryKey: ["services"], queryFn: fetchServices, initialData: STATIC_SERVICES, staleTime: Infinity, initialDataUpdatedAt: 0, initialDataUpdatedAt: 0 });
  const { data: settings = STATIC_SETTINGS } = useQuery({ queryKey: ["settings"], queryFn: fetchSettings, initialData: STATIC_SETTINGS, staleTime: Infinity, initialDataUpdatedAt: 0, initialDataUpdatedAt: 0 });
